...
@test "checking dovecot: debug mode enabled (traefik_acmev1 configuration)" {
  ...
  assert_success
  assert_output "yes"
  run docker exec mailserver_traefik_acmev1 /bin/sh -c "doveconf -h log_debug 2>/dev/null"
  assert_success
...
# fixtures_traefik_acmev1 rewrites acme.json once. A watcher that reacted to
...
@test "checking traefik acme v1: one write to acme.json causes exactly one reload" {
  run docker logs mailserver_traefik_acmev1
  assert_success
